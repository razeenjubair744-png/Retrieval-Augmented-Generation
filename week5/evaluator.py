...
def get_dataset_stats():
    ...
    if os.path.exists(kb_path):
        for folder in os.listdir(kb_path):
            folder_path = os.path.join(kb_path, folder)
            if os.path.isdir(folder_path):
                num_files = len([f for f in os.listdir(folder_path) if f.endswith('.txt')])
                kb_stats.append({"Category": folder.capitalize(), "Documents": num_files})
    
    # 2. Evaluation Dataset Stats
    tests_path = "evaluation/tests.jsonl"
    eval_stats_dict = defaultdict(int)
    if os.path.exists(tests_path):
        with open(tests_path, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    data = json.loads(line)
                    eval_stats_dict[data.get('category', 'unknown')] += 1
                except:
                    pass
    
    eval_stats = [{"Test Category": k.replace('_', ' ').title(), "Count": v} for k, v in eval_stats_dict.items()]
    
    return pd.DataFrame(kb_stats), pd.DataFrame(eval_stats)
    ...
    with gr.Blocks(title="RAG Evaluation Dashboard", theme=theme) as app:
        ...
        with gr.Tabs():
            with gr.Tab("⚙️ Settings"):
                ...
                gr.Markdown("Please enter your OpenAI API key to run evaluations. This ensures the default key is not exhausted.")
                ...
                def save_api_key(key):
                    if key and len(key) > 10:
                        ...
                        return "✅ API Key saved successfully! You can now run evaluations in the Dashboard tab."
            ...
            with gr.Tab("📈 Dataset Overview"):
                gr.Markdown("## 🗂️ Knowledge Base Composition")
                gr.Markdown("This chart shows the number of source documents (`.txt` files) in each section of the knowledge base.")
                kb_df, eval_df = get_dataset_stats()
                ...
                with gr.Row():
                    gr.BarPlot(
                        ...
                        x="Category",
                        y="Documents",
                        title="Documents per Knowledge Base Section",
                        height=350,
                    )
                
                gr.Markdown("## 📋 Evaluation Dataset")
                gr.Markdown("This chart shows the distribution of evaluation questions by category inside `tests.jsonl`.")
                ...
                with gr.Row():
                    gr.BarPlot(
                        ...
                        x="Test Category",
                        y="Count",
                        title="Evaluation Questions by Category",
                        height=350,
                    )
